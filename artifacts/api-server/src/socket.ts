...
const SEED_DATA_DIR  = join(process.cwd(), "artifacts/api-server/data");
...
function loadProfiles(): Map<string, UserProfile> {
  try {
    const filePath = existsSync(PROFILES_PATH) ? PROFILES_PATH : join(SEED_DATA_DIR, "profiles.json");
    if (existsSync(filePath)) {
      const raw = JSON.parse(readFileSync(filePath, "utf-8")) as Record<string, UserProfile>;
...
function loadRooms(): Map<string, Room> {
  try {
    const filePath = existsSync(ROOMS_PATH) ? ROOMS_PATH : join(SEED_DATA_DIR, "rooms.json");
    if (existsSync(filePath)) {
      const raw = JSON.parse(readFileSync(filePath, "utf-8")) as PersistedRoom[];
      const now = Date.now();
...
export function setupSocketIO(io: SocketIOServer) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("audio-pcm", (pcmData: ArrayBuffer) => {
      const roomId = socketToRoom.get(socket.id);
      if (!roomId) return;
      socket.to(VR + roomId).emit("audio-pcm", { fromId: socket.id, data: pcmData });
    });
    ...
    socket.on("owner-mute", (d: { targetId: string; muted: boolean }) => {
      ...
      if (!target || !roomId || target.isOwner) return;
    ...
    socket.on("owner-kick", (d: { targetId: string }) => {
      ...
      const targetSocket = io.sockets.sockets.get(d.targetId);
      if (targetSocket) {
        leaveRoom(targetSocket, roomId);
        targetSocket.emit("kicked", {});
      } else {
        room.participants.delete(d.targetId);
        io.to(VR + roomId).emit("participant-left", { participantId: d.targetId });
        io.emit("room-updated", { roomId, participantCount: room.participants.size, expiresAt: room.expiresAt });
      }
      logger.info({ owner: socketToName.get(socket.id), targetId: d.targetId, roomId }, "Participant kicked by owner");
    });
    ...
    socket.on("toggle-stealth", (d?: { stealthy?: boolean }) => {
      if (!isOwnerSocket(socket.id)) return;
      const roomId = socketToRoom.get(socket.id);
      const room   = roomId ? rooms.get(roomId) : null;
      const name   = socketToName.get(socket.id);
      if (!room || !roomId || !name) return;

      const currentlyStealth = stealthSet.has(socket.id);
      const newStealth = d?.stealthy !== undefined ? !!d.stealthy : !currentlyStealth;

      if (newStealth && !currentlyStealth) {
        stealthSet.add(socket.id);
        room.participants.delete(socket.id);
        socket.to(VR + roomId).emit("participant-left", { participantId: socket.id });
        io.emit("room-updated", { roomId, participantCount: room.participants.size, expiresAt: room.expiresAt });
        socket.emit("stealth-changed", { isStealth: true });
      } else if (!newStealth && currentlyStealth) {
        stealthSet.delete(socket.id);
        const p: Participant = {
          id: socket.id,
          name,
          ...
          isRM: false,
          isSpeaking: false,
          isRoomCreator: name === room.createdBy,
        };
        room.participants.set(socket.id, p);
        socket.to(VR + roomId).emit("participant-joined", sanitizeP(p));
        io.emit("room-updated", { roomId, participantCount: room.participants.size, expiresAt: room.expiresAt });
        socket.emit("stealth-changed", { isStealth: false });
      }
    });
